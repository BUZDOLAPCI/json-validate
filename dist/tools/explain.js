 ...
 */
function getKeywordExplanation(keyword, params, path) {
    ...
    switch (keyword) {
        ...
        case 'enum':
            const allowedValues = params.allowedValues?.map(v => JSON.stringify(v)).join(', ') || 'specified values';
            return {
        ...
        case 'dependentRequired':
            return {
                explanation: `The property at ${location} requires additional properties: ${params.deps?.join(', ') || params.missingProperty}.`,
 ...
 */
export function explainValidation(input) {
    try {
        // Validate input
        if (!input.errors || !Array.isArray(input.errors)) {
            return createErrorResponse('INVALID_INPUT', 'Errors must be an array of validation error objects', { received: typeof input.errors });
        }
        if (input.errors.length === 0) {
            return createSuccessResponse({ explanations: [] }, { source: 'json-validate' });
        }
        const explanations = input.errors.map((error) => {
            const normalized = normalizeError(error);
            const { explanation, suggestion } = getKeywordExplanation(normalized.keyword, normalized.params, normalized.path);
            return {
        ...
        return createErrorResponse('INTERNAL_ERROR', `Failed to explain errors: ${err instanceof Error ? err.message : String(err)}`, { error: err instanceof Error ? err.stack : undefined });
    }
}
