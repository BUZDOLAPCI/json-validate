 ...
 */
function tryParseJson(text: string): { parsed: unknown; errors: string[] } {
  const errors: string[] = [];
...
function deleteAtPath(obj: unknown, path: string): boolean {
  ...
  if (current !== null && current !== undefined && typeof current === 'object') {
    const lastPart = parts[parts.length - 1];
    if (lastPart in (current as Record<string, unknown>)) {
 ...
 */
function tryCoerce(value: unknown, targetType: string): { coerced: unknown; success: boolean } {
 ...
 */
function getSchemaAtPath(schema: Record<string, unknown>, path: string): Record<string, unknown> | undefined {
...
function removeAdditionalProperties(
  ...
  if (schema.type === 'object' || schema.properties) {
    ...
    const patternProps = schema.patternProperties as Record<string, Record<string, unknown>> | undefined;
    ...
    for (const key of Object.keys(objRecord)) {
      ...
      const matchesPattern = patternProps && Object.keys(patternProps).some(pattern =>
...
function applyDefaults(
  ...
  if (schema.type === 'array' && schema.items) {
    ...
    return obj.map((item, index) =>
      applyDefaults(item, itemSchema, `${path}/${index}`, changes)
    );
  }
...
function applyTypeCoercion(
  ...
  if (types.includes(currentType)) {
    ...
    if (currentType === 'object' && schema.properties && obj !== null) {
      ...
      for (const [propName, propSchema] of Object.entries(properties)) {
        if (propName in objRecord) {
          objRecord[propName] = applyTypeCoercion(
            objRecord[propName],
            propSchema,
            `${path}/${propName}`,
            changes
      ...
      const itemSchema = schema.items as Record<string, unknown>;
      return obj.map((item, index) =>
        applyTypeCoercion(item, itemSchema, `${path}/${index}`, changes)
      );
    }
    return obj;
  }
 ...
 */
export function repairJson(input: RepairJsonInput): ResponseEnvelope<RepairResult> {
  try {
    ...
    if (typeof input.instance_or_text === 'string') {
      ...
      if (parsed === undefined) {
        return createErrorResponse(
          'PARSE_ERROR',
          'Could not parse or repair JSON string',
          { parseErrors }
        );
      }
    ...
    return createSuccessResponse<RepairResult>(
      {
        repaired: instance,
        changes,
